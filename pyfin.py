...
#  Bereken maandelikse onttrekkings/bydraes oor 12 maande.
#  Bou inflasie in
#  Bou bounds in
...
#  Skryf White Paper
#  Heroku
#  Bou veranderlike in vir lump sum amount (tot en met 30%)
...
class Portfolio(object):
    ...
    def fractionsToRands(self, ind):
        ...
        '''
        contr = np.zeros_like(ind)
        ra_contr = ind[:, :len(self.ra_list)].sum(axis=1)
    ...
    def objective(self, individual):
        ...
        for count, i in enumerate(self.investments.keys()):
            self.investments[i].calculateOptimalWithdrawal(self.contr.loc[:, count], self.strategy)
    ...
    def mutatePyfin(self, individual, indpb, number_changed):
        ...
        return creator.Individual(reshaped.reshape(reshaped.size)),
        #return reshaped.reshape(reshaped.size),
...
class TFSA(Investment):
    ...
    def calculate(self):
        ...
        for year in self.df.index[1:]:
            ...
            self.df.loc[year, 'capital'] = self.df.loc[previous_year, 'capital']*(1 + self.growth) + contr
    ...
    def _calculateQuick(self, arr, growth, withdrawal):
        ...
        '''Quick version of calculate, simply for determined optimal drawdown rate.
        ...
        for i in range(1, arr.shape[0]): 
            arr[i, 0] = arr[i - 1, 0]*(1 + growth) - withdrawal
        return arr[-1, 0]  
...
class RA(Investment):
    ...
    def calculateOptimalWithdrawal(self, contr, strategy='optimal'):
        self.growthBeforeRetirement(contr)
        self.growthAfterRetirementOptimalWithdrawal(contr, strategy)
        
    def growthBeforeRetirement(self, contr):
        
        previous_year = self.df.index[0]
        self.df['contr'] = contr    
        
        for year in self.df.loc[self.df.index<self.retirement_date].index[1:]:
            self.df.loc[year, 'capital'] = self.df.loc[previous_year, 'capital']*(1 + self.ra_growth) + self.df.loc[year, 'contr']
            if year.year == 3:
        ...
        #  Also,  
        self.df.loc[self.first_retirement_date, 'capital'] = capital_at_retirement*(2/3)

    def growthAfterRetirementOptimalWithdrawal(self, contr, strategy='optimal'):
    ...
    def _growthAfterRetirementQuick(self, arr, growth, withdrawal):
            for i in range(1, len(arr)):  
                ...
                arr[i] = arr[i - 1]*(1 + growth) - withdrawal_i
            return arr[-1]  
...
class DI(Investment):
    ...
    def recalculateOptimalWithdrawal(self, strategy='optimal'):
        ...
        if strategy == 'optimal':
            capital_at_le = 1e7
            arr = self.df.loc[self.last_working_date:, ['capital', 'contr']].values
            while capital_at_le > 0:
                drawdown += 0.001
    ...
    def _calculateQuick(self, arr, growth, withdrawal):        
        ...
        for i in range(1, arr.shape[0]):
            arr[i, 0] = arr[i - 1, 0]*(1 + growth) + arr[i, 1] - withdrawal
        #print(arr)
        return  arr[-1, 0]        
...
contr_TFSA = pd.Series(index=tfsa.df.index, name='contr',
                       data=100000*np.ones(tfsa.df.shape[0]))
